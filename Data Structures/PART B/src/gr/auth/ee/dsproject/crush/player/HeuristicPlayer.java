 ...
 * 
 * <p>
 * Description: Creating a 'clever' player that evaluate every available move and choose the best of them, using an evaluation function.
 * </p>
 * 
...
public class HeuristicPlayer implements AbstractPlayer
{
  ...
  double moveEvaluation (int[] move, Board board)
  {
	...
	//Get deleted candies ( * 2 beacause it's the most important criterion
	evaluation += deletedCandies(move,newBoard) * 2;
	//System.out.println("Deleted candies evaluation (final): " + evaluation);
  ...
  double calculateSameColorInProximityCandies (int [] move, Board board){
	  ...
	  //Calculate candeis same as destination tile of move
	  if (move[2] == CrushUtilities.UP) {
  ...
  ArrayList<Tile> deletedCandiesAtRow (Board board, int row){
	  ...
	  for (int x = 0; x < CrushUtilities.NUMBER_OF_COLUMNS - 2; x++){ 
		  ...
		  while ( (x + compareIndex) < CrushUtilities.NUMBER_OF_COLUMNS  &&  sameTiles.get(0).getColor() == board.giveTileAt(x + compareIndex, row).getColor() ){
  ...
  ArrayList<Tile> deletedCandiesAtColumn (Board board, int column){
	  ...
	  for (int y = 0; y < CrushUtilities.NUMBER_OF_PLAYABLE_ROWS - 2; y++){ 
		  ...
		  //in order to find the same candies 
		  //Also check if (x + compareIndex) is valid (into the board)
		  while ( (y + compareIndex) < CrushUtilities.NUMBER_OF_COLUMNS  &&  sameTiles.get(0).getColor() == board.giveTileAt(column, y + compareIndex).getColor() ){
  ...
  int deletedCandies (int[] move, Board board){
	  ...
	  //System.out.println("Tile y:" + move[1]);
