...
//Calculate knn using ring network
void knn_no_block(){
  int i;
...
//Initialize the k_dist with DBL_MAX
void init_dist(){
  int i, j;
...
//Apply the main knn procedure at a specific process
...
int validate_parallel(){
  ...
  if (PID == 1){
    ...
    //get the new k_dist that includes the next data and test again
